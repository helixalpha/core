...
const char * const BITCOIN_CONF_FILENAME = "helixalpha.conf";
const char * const BITCOIN_PID_FILENAME = "helixalpha.pid";
...
#else
    const char* pszModule = "helixalpha";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HelixAlpha";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HelixAlpha";
#else
    // Unix
    return pathRet / ".helixalpha";
#endif
#endif
}
